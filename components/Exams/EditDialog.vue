  ...
  >
    <v-card class="pa-sm-7 pa-5">
      <v-card-title class="px-0 text-center text-wrap font-weight-bold">
        Редактировании экзамена/консультации
      </v-card-title>

      <v-card-text class="px-0">
        <v-slide-x-transition>
          <v-alert type="error" class="mb-6" v-if="isEditError">
            <p class="text-h6">
              Произошла ошибка при обновлении экзамена, повторите попытку позже
            </p>
          </v-alert>
        </v-slide-x-transition>

        <v-form @submit="editExamSubmit">
          <v-row>
            <v-col cols="12">
              <v-text-field
                v-model="className"
...
const initialFormValues = computed(() => {
  return {
    ...props.exam,
    date: formatDate(props.exam.date),
  };
});

const { handleSubmit, handleReset, isSubmitting } = useForm<SessionExamForm>({
...
const editExamSubmit = handleSubmit(async (examData: SessionExamForm) => {
  try {
